...
public abstract partial class ResolvedSoundSpecifier {
    ...
    [Obsolete("Implicit string conversion for resolved specifiers are deprecated, store the ResolvedSoundSpecifier instead")]
    public static implicit operator string(ResolvedSoundSpecifier? specifier) =>
        specifier switch {
            ResolvedPathSpecifier path => path.Path.ToString(),
            ResolvedCollectionSpecifier collection => collection.Path.ToString(),
            null => null!,
        };
...
public sealed partial class ResolvedCollectionSpecifier : ResolvedSoundSpecifier {
    ...
    /// The resource path of the sound.
    /// </summary>
    [Obsolete("Path for ResolvedCollectionSpecifier is deprecated, look up the collection prototype instead")]
    public ResPath Path { get; private set; }
    ...
    public ResolvedCollectionSpecifier(string collection, int index, ResPath path)
    {
        Collection = collection;
        Index = index;
        Path = path;
    }
    public ResolvedCollectionSpecifier(string collection, int index, string path) : this(collection, index, new ResPath(path))
    {
    }
}
