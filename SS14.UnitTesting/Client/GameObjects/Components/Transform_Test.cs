...
";
        ...
        public void ParentAttachMoveTest()
        {
            ...
            var compState = new TransformComponentState(new Vector2(5, 5), new GridId(5), new Angle(0), EntityUid.Invalid);
            parentTrans.HandleComponentState(compState);

            compState = new TransformComponentState(new Vector2(6, 6), new GridId(5), new Angle(0), EntityUid.Invalid);
            childTrans.HandleComponentState(compState);
            // World pos should be 6, 6 now.

            // Act
            var oldWpos = childTrans.WorldPosition;
            compState = new TransformComponentState(Vector2.One, new GridId(5), new Angle(0), parent.Uid);
            childTrans.HandleComponentState(compState);
        ...
        public void WorldRotationTest()
        {
            ...
            var compState = new TransformComponentState(new Vector2(6, 6), new GridId(5), Angle.FromDegrees(135), EntityUid.Invalid);
            node1Trans.HandleComponentState(compState);
            compState = new TransformComponentState(new Vector2(1, 1), new GridId(5), Angle.FromDegrees(45), node1.Uid);
            node2Trans.HandleComponentState(compState);
            compState = new TransformComponentState(Vector2.Zero, new GridId(5), Angle.FromDegrees(45), node2.Uid);
