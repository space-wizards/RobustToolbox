...
public class Generator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor CustomTypeSerializerInterfacesRule = new(
        Diagnostics.IdCustomTypeSerializerInterfacesRule,
        "Custom type serializer must implement ITypeCopier<T> or ITypeCopyCreator<T>",
        "Custom type serializer {0} for data field {1} in data definition {2} does not implement neither the ITypeCopier<T> nor the ITypeCopyCreator<T> interfaces.",
        "Usage",
        DiagnosticSeverity.Error,
        true,
        "Make sure to implement either ITypeCopier<T> or ITypeCopyCreator<T> in the custom type serializer."
    );
...
""");
                    ...
                    var definition = GetDataFields(symbol, sourceContext);
...
"""
    ...
    private static DataDefinition GetDataFields(ITypeSymbol definition, SourceProductionContext context)
    {
        ...
        return new DataDefinition(definition, typeName, fields, otherFields);
    }
