...
namespace Robust.Shared.Localization
{
    internal abstract partial class LocalizationManager
    {
        ...
        private void AddBuiltInFunctions(FluentBundle bundle)
        {
            ...
            foreach (var tense in _protoMan.EnumeratePrototypes<PronounTense>())
                if (tense.FtlFunction is { } function &&
                    tense.FtlReturn is { })
                    AddCtxFunction(bundle, function, FuncAutoGenerate, tense);
        ...
        /// </summary>
        private ILocValue FuncAutoGenerate(LocArgs args)
        {
            var outGender = "";
            if (args.Tense is not { } tense ||
                tense.FtlFunction is not { } function) // this should not happen
            ...
            else if (args.Args[0].Value is EntityUid entity)
            {
                ...
                {
                    if (grammar.Pronoun.TryGetValue(tense, out var value))
                        outGender = value;
        ...
        private void AddCtxFunction(FluentBundle ctx, string name, LocFunction function, PronounTense? tense = null)
        {
            ctx.AddFunctionOverriding(name, (args, options)
                => CallFunction(function, ctx, args, options, tense));
        }
            ...
            PronounTense? tense)
        {
            ...
            var argStruct = new LocArgs(args, options, tense);
            return function.Invoke(argStruct).FluentFromVal(new LocContext(bundle));
        }

        public void AddFunction(CultureInfo culture, string name, LocFunction function, PronounTense? tense = null)
        {
            var bundle = _contexts[culture];

            bundle.AddFunctionOverriding(name, (args, options)
                => CallFunction(function, bundle, args, options, tense));
        }
    }
