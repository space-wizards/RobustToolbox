...
[PublicAPI]
public readonly struct EntityQuery<TComp1, TComp2> : IEnumerable<Entity<TComp1, TComp2>>
    ...
    where TComp2 : IComponent
{
    ...
    /// </code>
    /// </example>
    public EntityQuery<TComp1, TComp2> All => new(this, true);

    internal EntityQuery(DynamicEntityQuery query, EntityManager entMan)
    {
        DebugTools.AssertEqual(query.OutputCount, 2);
        _query = query;
        _entMan = entMan;
    ...
    /// </summary>
    private EntityQuery(EntityQuery<TComp1, TComp2> derived, bool enumeratePaused)
    {
    ...
    public bool TryGet(EntityUid ent, [NotNullWhen(true)] out TComp1? comp1, [NotNullWhen(true)] out TComp2? comp2)
    {
    ...
    /// <returns>True when all components were found, false otherwise.</returns>
    public bool TryGet(EntityUid ent, out Entity<TComp1, TComp2> resolved)
    {
        if (TryGet(ent, out var c1, out var c2))
        {
            resolved = new(ent, c1, c2);
            return true;
        }
    ...
    /// <returns>True when all components were found, false otherwise.</returns>
    public bool Resolve(EntityUid ent, [NotNullWhen(true)] ref TComp1? comp1, [NotNullWhen(true)] ref TComp2? comp2, bool logMissing = true)
    {
        var buffer = new ComponentArray();
        buffer[0] = comp1;
        buffer[1] = comp2;
        ...
        if (logMissing)
            _entMan.ResolveSawmill.Error($"Can't resolve \"{typeof(TComp1)}\" and \"{typeof(TComp2)}\" on entity {_entMan.ToPrettyString(ent)}!\n{Environment.StackTrace}");
    ...
    /// <param name="logMissing">Whether to log if the resolve fails.</param>
    /// <returns>True when all components were found, false otherwise.</returns>
    public bool Resolve(ref Entity<TComp1?, TComp2?> entity, bool logMissing = true)
    {
    ...
    IEnumerator<Entity<TComp1, TComp2>> IEnumerable<Entity<TComp1, TComp2>>.GetEnumerator()
    {
    ...
    public struct Enumerator : IEnumerator<Entity<TComp1, TComp2>>
    {
        private DynamicEntityQuery.Enumerator _enumerator;
        public Entity<TComp1, TComp2> Current { get; private set; }

        internal Enumerator(EntityQuery<TComp1, TComp2> owner)
        {
        ...
        public bool MoveNext()
        {
            ...
            {
                Current = new(ent, (TComp1)buffer[0]!, (TComp2)buffer[1]!);
                return true;
            }
        ...
        Entity<TComp1, TComp2> IEnumerator<Entity<TComp1, TComp2>>.Current => Current;
