    ...
    public class SnapgridCenter : PlacementMode
    {
        ...
        public override void Render()
        {
            ...
            if (ongrid)
            {
                ...
                (float)(Math.Round(((position.X / snapsize)-snapsize/2), MidpointRounding.AwayFromZero)+snapsize/2) * snapsize,
                (float)(Math.Round(((position.Y / snapsize)-snapsize/2), MidpointRounding.AwayFromZero)+snapsize/2) * snapsize));
        ...
        public override bool Update(Vector2i mouseS, IMapManager currentMap)
        {
            ...
            mouselocal = new Vector2( //Round local coordinates onto the snap grid
                (float)(Math.Round(((mouselocal.X / snapsize)-snapsize/2), MidpointRounding.AwayFromZero)+snapsize/2) * snapsize, 
                (float)(Math.Round(((mouselocal.Y / snapsize)-snapsize/2), MidpointRounding.AwayFromZero)+snapsize/2) * snapsize);
