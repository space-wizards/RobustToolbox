  ...
  - type: Transform
    anchored: true";
        ...
        public void OnAnchored_WorldPosition_TileCenter()
        {
            ...
            // Act
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Anchored = true;

            Assert.That(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).WorldPosition, Is.EqualTo(new Vector2(7.5f, 7.5f))); // centered on tile
        ...
        public void OnAnchored_Parent_SetToGrid()
        {
            ...
            Assert.That(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).ParentUid, Is.EqualTo(grid.GridEntityId));
        ...
        public void OnAnchored_EmptyTile_Nop()
        {
            ...
            var tileIndices = grid.TileIndicesFor(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Coordinates);
        ...
        public void Anchored_SetPosition_Nop()
        {
            ...
            int calledCount = 0;
            var ent1 = entMan.SpawnEntity(null, coordinates); // this raises MoveEvent, subscribe after
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Anchored = true; // Anchoring will change parent if needed, raising MoveEvent, subscribe after
            entMan.EventBus.SubscribeEvent<MoveEvent>(EventSource.Local, subscriber, MoveEventHandler);

            // Act
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).WorldPosition = new Vector2(99, 99);
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).LocalPosition = new Vector2(99, 99);
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Coordinates = new EntityCoordinates(grid.GridEntityId, 99, 99); // make sure not to change parent, that would un-anchor

            Assert.That(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).MapPosition, Is.EqualTo(coordinates));
        ...
        public void Anchored_ChangeParent_Unanchors()
        {
            ...
            // Act
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).ParentUid = mapMan.GetMapEntityId(TestMapId);

            Assert.That(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Anchored, Is.False);
            Assert.That(grid.GetAnchoredEntities(tileIndices).Count(), Is.EqualTo(0));
        ...
        public void Anchored_SetParentSame_Nop()
        {
            ...
            // Act
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).ParentUid = grid.GridEntityId;
        ...
        public void OnUnanchored_HasPhysicsComp_IsDynamicBody()
        {
            ...
            // Act
            IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).Anchored = false;
        ...
        public void Unanchored_Unanchor_Nop()
        {
            ...
            Assert.That(IoCManager.Resolve<IEntityManager>().GetComponent<TransformComponent>(ent1.Uid).ParentUid, Is.EqualTo(mapMan.GetMapEntityId(TestMapId)));
            Assert.That(calledCount, Is.EqualTo(0));
            void ParentChangedHandler(ref EntParentChangedMessage ev)
            {
                Assert.That(ev.Entity, Is.EqualTo(ent1));
