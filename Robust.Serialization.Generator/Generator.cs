...
public class Generator : IIncrementalGenerator
{
    ...
    private static DataDefinition GetDataDefinition(ITypeSymbol definition)
    {
        ...
        var supportsPrototypeCopy = definition.TypeKind != TypeKind.Interface &&
                                    !hasHooks &&
                                    ImplementsInterface(definition, ComponentInterfaceName);

        return new DataDefinition(definition, typeName, fields, hasHooks, invalidFields, supportsPrototypeCopy);
    }
    ...
    {
        if (!definition.SupportsPrototypeCopy)
            return string.Empty;
        ...
        if (baseType != null && IsDataDefinition(baseType))
        {
            var baseName = baseType.ToDisplayString();
            baseCall = $"""
                        var definitionCast = ({baseName}) target;
