    ...
    public class LineEdit : Control
    {
        ...
        protected internal override void KeyBindDown(GUIBoundKeyEventArgs args)
        {
            ...
            if (!args.CanFocus)
            {
                ...
                if (args.Function == EngineKeyFunctions.MoveWordRight)
                {
                    if (!this.HasKeyboardFocus())
                    {
                        return;
                    ...
                    var nextBoundary = _getNextWordBoundary();
                    _cursorPosition = nextBoundary;
                }

                else if (args.Function == EngineKeyFunctions.MoveWordLeft)
                {
                    if (!this.HasKeyboardFocus())
                    {
                        return;
                    ...
                    var nextBoundary = _getNextWordBoundary() - 2;
                    if (nextBoundary < 0)
                    {
                        nextBoundary = 0;
                    }
                ...
                if (args.Function == EngineKeyFunctions.SelectWordRight)
                {
                    if (!this.HasKeyboardFocus())
                    {
                        return;
                    ...
                    var nextBoundary = _getNextWordBoundary();
                    Select(_cursorPosition, nextBoundary);
                    _cursorPosition = nextBoundary;
                }

                if (args.Function == EngineKeyFunctions.SelectWordLeft)
                {
                    if (!this.HasKeyboardFocus())
                    {
                        return;
        ...
        private int _getNextWordBoundary()
        {
            // Calculate word boundaries in _text, return first word boundary past _cursorPosition
