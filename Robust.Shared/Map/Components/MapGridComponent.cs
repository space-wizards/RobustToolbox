...
namespace Robust.Shared.Map.Components
{
    ...
    public sealed class MapGridComponent : Component
    {
        ...
        /// <inheritdoc />
        public IEnumerable<TileRef> GetTilesOutline(Circle worldArea, bool ignoreEmpty = true,
            Predicate<TileRef>? predicate = null)
        {
            var matrix = _entMan.GetComponent<TransformComponent>(Owner).InvWorldMatrix;
            var radius = worldArea.Radius;
            var center = matrix.Transform(worldArea.Position);
            // Center the center to the center of the tile
            center = new Vector2(MathF.Floor(center.X / TileSize) + TileSize / 2f, MathF.Floor(center.Y / TileSize) + TileSize / 2f);
            var tiles = new HashSet<TileRef>();
            var adjustedRadius = worldArea.Radius / TileSize;

            Span<Vector2i> radiusTiles = stackalloc Vector2i[8];

            for (var r = 0; r <= MathF.Floor(radius * MathF.Sqrt(0.5f)); r++)
            {
                var d = MathF.Floor(MathF.Sqrt(adjustedRadius * adjustedRadius - r * r));

                radiusTiles[0] = new Vector2i((int)(center.X - d), (int)(center.Y + r));
                radiusTiles[1] = new Vector2i((int) (center.X + d), (int) (center.Y + r));
                radiusTiles[2] = new Vector2i((int) (center.X - d), (int) (center.Y - r));
                radiusTiles[3] = new Vector2i((int) (center.X + d), (int) (center.Y - r));
                radiusTiles[4] = new Vector2i((int) (center.X + r), (int) (center.Y - d));
                radiusTiles[5] = new Vector2i((int) (center.X + r), (int) (center.Y + d));
                radiusTiles[6] = new Vector2i((int) (center.X - r), (int) (center.Y - d));
                radiusTiles[7] = new Vector2i((int) (center.X - r), (int) (center.Y + d));

                for (var i = 0; i < 8; i++)
                {
                    if (TryGetTile(radiusTiles[i], ignoreEmpty, out var tileRef, predicate))
                    {
