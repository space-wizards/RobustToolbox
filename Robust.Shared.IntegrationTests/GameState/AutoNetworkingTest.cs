...
internal sealed partial class AutoNetworkingTests : RobustIntegrationTest
{
    ...
    public async Task AutoNetworkingFieldDeltaFromTickBoundaryTest()
    {
        ...
        Assert.That(state, Is.TypeOf<AutoNetworkingTestFieldDeltaComponent.AutoNetworkingTestFieldDeltaComponent_AutoDeltaState>());
        var delta = (IAutoGeneratedComponentDeltaState) state!;
        Assert.That(delta.ChangedFields, Is.EqualTo(1UL << 1));
    }
