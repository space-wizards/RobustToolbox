...
namespace ClientServices.State.States
{
    public class NewLobby : State, IState
    {
        ...
        private SimpleImage _imgStatus;
        ...
        private float _lastLblSpacing = 10; //TODO Actually calculate this and adjust all labels accordingly. Make sure we compensate if labels longer than status line.

        private List<Label> _serverLabels = new List<Label>();

        private JobTab _tabJob;
        private TabContainer _tabCharacter;
        private TabContainer _tabObserve;
        private TabContainer _tabServer;
        private TabbedMenu _tabs;

        private TabContainer _tabActive;

        private RectangleF _recStatus = new RectangleF();
        ...
        public void Startup()
        {
            ...
            foreach(var curr in _serverLabels)
        ...
        public void Update(FrameEventArgs e)
        {
            ...
            _lblMode.Position = new Point(_lblServerInfo.ClientArea.Right + (int)_lastLblSpacing, _lblServerInfo.ClientArea.Y);
            _lblModeInfo.Position = new Point(_lblMode.ClientArea.Right, _lblMode.ClientArea.Y);

            _lblPlayers.Position = new Point(_lblModeInfo.ClientArea.Right + (int)_lastLblSpacing, _lblModeInfo.ClientArea.Y);
            _lblPlayersInfo.Position = new Point(_lblPlayers.ClientArea.Right, _lblPlayers.ClientArea.Y);

            _lblPort.Position = new Point(_lblPlayersInfo.ClientArea.Right + (int)_lastLblSpacing, _lblPlayersInfo.ClientArea.Y);
