...
public sealed class BroadphaseNetworkingTest : RobustIntegrationTest
{
    ...
    public async Task TestBroadphaseNetworking()
    {
        ...
        var map1Net = sEntMan.ToNetEntity(map1);
        ...
        var playerNet = sEntMan.ToNetEntity(player);
        ...
        var cPlayerXform = cEntMan.GetComponent<TransformComponent>(cEntMan.ToEntity(playerNet));

        // Client initially has correct transform data.
        var broadphase = new BroadphaseData(grid1, map1, true, false);
        var grid1Net = sEntMan.ToNetEntity(grid1);

        Assert.That(cPlayerXform.GridUid, Is.EqualTo(cEntMan.ToEntity(grid1Net)));
        Assert.That(sPlayerXform.GridUid, Is.EqualTo(grid1));
        Assert.That(cPlayerXform.MapUid, Is.EqualTo(cEntMan.ToEntity(map1Net)));
        Assert.That(sPlayerXform.MapUid, Is.EqualTo(map1));

        Assert.That(cPlayerXform.Broadphase?.Uid, Is.EqualTo(cEntMan.ToEntity(sEntMan.ToNetEntity(broadphase.Uid))));
        Assert.That(cPlayerXform.Broadphase?.PhysicsMap, Is.EqualTo(cEntMan.ToEntity(sEntMan.ToNetEntity(broadphase.PhysicsMap))));
        ...
        var grid2Net = sEntMan.ToNetEntity(grid2);
        var map2Net = sEntMan.ToNetEntity(map2);
        ...
        Assert.That(cEntMan.ToNetEntity(cPlayerXform.GridUid), Is.EqualTo(grid2Net));
        Assert.That(sPlayerXform.GridUid, Is.EqualTo(grid2));
        Assert.That(cEntMan.ToNetEntity(cPlayerXform.MapUid), Is.EqualTo(map2Net));
