...
internal partial class Clyde
{
    ...
    private const float MaxLightRadius = 20.0f;

    // As exposure grows above 1.0, the radius of lights increases, so we need to start looking further to find
    //   relevant lights
    private const float LargerQueryThreshold = 1.5f;

    // How many units further we look for lights for each 1.0 increase of Exposure.
    private const float LargerQueryArea = 5f;
        ...
        in ComponentTreeEntry<PointLightComponent> value)
    {
        ...
        light.Radius = Math.Min(MaxLightRadius, light.Radius * rangeMul);
