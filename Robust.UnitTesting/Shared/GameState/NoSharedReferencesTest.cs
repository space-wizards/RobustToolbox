...
public sealed partial class NoSharedReferencesTest : RobustIntegrationTest
{
    ...
    /// </summary>
    [Test]
    [Ignore("Enable this test when RobustIntegrationTest are changed to not send references via netcode")]
    public async Task ReferencesAreNotShared()
    {
        void compReg() => IoCManager.Resolve<IComponentFactory>().RegisterClass<ExampleAutogeneratedComponent>();
        void sysReg() => IoCManager.Resolve<IEntitySystemManager>().LoadExtraSystemType<ExampleAutogeneratedComponent_AutoNetworkSystem>();
        ...
        Assert.Multiple(() =>
        {
            ...
            var clientLastTickStateObject = ((ExampleAutogeneratedComponent_AutoState)componentStates!.First(x => x.Value.GetType() == typeof(ExampleAutogeneratedComponent_AutoState)).Value)?.ReferenceObject;
