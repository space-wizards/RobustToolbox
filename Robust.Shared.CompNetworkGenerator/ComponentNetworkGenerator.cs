...
");
    ...
    public GameTick[] LastModifiedFields {{ get; set; }}";
            ...
            if (deltaFieldsOnly)
            {
                var outDeltaSb = new StringBuilder();

                // Some components provide their own state objects and state handlers, but still need generated
                // IComponentDelta fields and networked field registration for DirtyField support.
                outDeltaSb.Append("""
                    // <auto-generated />
                    #nullable enable
                    using System;
                ...
                partialInfo.WriteHeader(outDeltaSb);
                ...
                outDeltaSb.Append($$"""
                    
                    {
                        ...
                        public sealed class {{componentName}}_AutoDeltaFieldSystem : EntitySystem
                        {
                            public override void Initialize()
                            {
                                EntityManager.ComponentFactory.RegisterNetworkedFields<{{componentName}}>({{fieldsStr}});
                            }
                        }
                    }
                    """);

                partialInfo.WriteFooter(outDeltaSb);

                return outDeltaSb.ToString();
            }
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var (classType, classSyntax, attribute) in symbols)
            {
                try
                {
                    ...
                    var deltaFieldsOnly = false;
                    if (attribute.ConstructorArguments.Length > 0 &&
                        attribute.ConstructorArguments[0].Value is bool raise)
                    {
                    ...
                    if (attribute.ConstructorArguments.Length > 1 &&
                        attribute.ConstructorArguments[1].Value is bool fields)
                    {
                        fieldDeltas = fields;
                    }

                    if (attribute.ConstructorArguments.Length > 2 &&
                        attribute.ConstructorArguments[2].Value is bool deltaOnly)
                        deltaFieldsOnly = deltaOnly;

                    var source = GenerateSource(context, classType, classSyntax, comp, raiseEv, fieldDeltas, deltaFieldsOnly);
