...
internal sealed class TextSelectionHelper
{
    ...
    /// <summary>
    ///     Selects the whole word at the specified index.
    /// </summary>
    public void SelectWord(string text, int index)
    {
        index = MathHelper.Clamp(index, 0, text.Length);
        var lower = TextEditShared.PrevWordPosition(text, index);
        var upper = TextEditShared.EndWordPosition(text, index);
        BeginSelection(lower, preserveAnchor: false);
        UpdateSelection(upper);
        EndSelection();
    }
