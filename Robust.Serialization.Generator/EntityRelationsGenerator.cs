        ...
        context.RegisterImplementationSourceOutput(componentInfos, static (productionContext, info) =>
        {
            ...
            foreach (var field in info.Fields)
            {
                ...
                if (field.Nullable)
                {
                    ...
                    shutdownBuilder.AppendLine($"    ClearRelation(ent.Owner, ref ent.Comp.{field.Name});");
                }
                else if (field.DictionaryKey)
                {
                    relationBuilder.AppendLine($"    ent.Comp.{field.Name}.Remove(args.Relation);");

                    shutdownBuilder.AppendLine($"    ClearRelation(ent.Owner, ent.Comp.{field.Name});");
                }
            ...
            result.AppendLine($$"""
                ...
                public sealed partial class {{info.PartialTypeInfo.Name}}_AutoRelationsSystem : EntitySystem
                {
                    ...
                    {
                        base.Initialize();
                        SubscribeLocalEvent<{{info.PartialTypeInfo.Name}}, ComponentRelationsRemove>(OnRelationRemove);
                    ...
                    private void OnRelationRemove(Entity<{{info.PartialTypeInfo.Name}}> ent, ref ComponentRelationsRemove args)
                    {
                    {{shutdownBuilder}}
                    }
                }
