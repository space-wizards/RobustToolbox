...
internal partial class MapManager
{
        ...
        [NotNullWhen(true)] out MapGridComponent? grid)
    {
        ...
        FindGridsIntersectingApprox(mapId, aabb, ref state, static (MapGridComponent grid, ref (MapGridComponent? grid, Vector2 worldPos, EntityQuery<TransformComponent> xformQuery) tuple) =>
        {
            ...
            var matrix = tuple.xformQuery.GetComponent(grid.Owner).InvWorldMatrix;
            ...
            var chunkIndices = grid.GridTileToChunkIndices(tile);

            if (!grid.HasChunk(chunkIndices)) return true;

            var chunk = grid.GetChunk(chunkIndices);
