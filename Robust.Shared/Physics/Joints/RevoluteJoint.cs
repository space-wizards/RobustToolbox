...
namespace Robust.Shared.Physics.Joints
{
    ...
    public class RevoluteJoint : Joint
    {
        ...
        public RevoluteJoint(PhysicsComponent bodyA, PhysicsComponent bodyB, Vector2 anchorA, Vector2 anchorB, bool useWorldCoordinates = false)
            : base(bodyA, bodyB)
        {
            JointType = JointType.Revolute;
        ...
        public RevoluteJoint(PhysicsComponent bodyA, PhysicsComponent bodyB, Vector2 anchor, bool useWorldCoordinates = false)
        ...
        {
            get { return BodyB.Sweep.Angle - BodyA.Sweep.Angle - ReferenceAngle; }
        }
        ...
        {
            get { return BodyB.AngularVelocity - BodyA.AngularVelocity; }
        }
        ...
        internal override void InitVelocityConstraints(ref SolverData data)
        {
            ...
            if (_enableLimit && fixedRotation == false)
            {
                ...
                if (Math.Abs(_upperAngle - _lowerAngle) < 2.0f * PhysicsSettings.AngularSlop)
                {
            ...
            if (data.Step.WarmStarting)
            {
                ...
                vA -= P * mA;
                wA -= iA * (Vector2.Cross(_rA, P) + MotorImpulse + _impulse.Z);

                vB += P * mB;
                wB += iB * (Vector2.Cross(_rB, P) + MotorImpulse + _impulse.Z);
            }
            else
            {
                _impulse = Vector3.Zero;
        ...
        internal override void SolveVelocityConstraints(ref SolverData data)
        {
            ...
            if (_enableLimit && _limitState != LimitState.Inactive && fixedRotation == false)
            {
                ...
                else if (_limitState == LimitState.AtLower)
                {
                    ...
                    if (newImpulse < 0.0f)
                    {
                        Vector2 rhs = -Cdot1 + new Vector2(_mass.ez.X, _mass.ez.Y) * _impulse.Z;
        ...
        internal override bool SolvePositionConstraints(ref SolverData data)
        {
            ...
            if (_enableLimit && _limitState != LimitState.Inactive && fixedRotation == false)
            {
                ...
                if (_limitState == LimitState.Equal)
                {
                    ...
                    float C = Math.Clamp(angle - _lowerAngle, -PhysicsSettings.MaxAngularCorrection, PhysicsSettings.MaxAngularCorrection);
                ...
                else if (_limitState == LimitState.AtLower)
                {
                    ...
                    C = Math.Clamp(C + PhysicsSettings.AngularSlop, -PhysicsSettings.MaxAngularCorrection, 0.0f);
                    limitImpulse = -_motorMass * C;
                }
                else if (_limitState == LimitState.AtUpper)
                {
                    ...
                    // Prevent large angular corrections and allow some slop.
                    C = Math.Clamp(C - PhysicsSettings.AngularSlop, 0.0f, PhysicsSettings.MaxAngularCorrection);
                    limitImpulse = -_motorMass * C;
                }
